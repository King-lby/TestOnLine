...
public class ExportcourseServlet extends HttpServlet {
	...
	//显示的导出表的标题
			...
			//导出表的列名
			private String[] rowsName= new String[]{"序号","课程号","课程名","课程老师工号","课程标记"};
		//	rowsName[] = new String[]{"序号","货物运输批次号","提运单号"};
			
			public List<Course>  dataList = new ArrayList<Course>();
		...
		public void doGet(HttpServletRequest request, HttpServletResponse response,List<Course> list) throws ServletException, IOException {
...
//			dataList=ex.find();
			...
			for (Course courseBean : dataList) {
				System.out.println(courseBean+"//");
			}
			ExportcourseServlet ex1 = new ExportcourseServlet();
		...
		public void doPost(HttpServletRequest request, HttpServletResponse response,List<Course> list) throws ServletException, IOException {
		 ...
		 * 导出数据
		 * */
		public void export(HttpServletRequest request, HttpServletResponse response,List<Course> dataList) throws Exception{
			try{
				...
				for(int i=0;i<dataList.size();i++){
					...
					Course obj = dataList.get(i);//遍历每个对象
...
//							cell.setCellValue(obj.getTab());
						...
						if(j==3){
							cell = row.createCell(j,HSSFCell.CELL_TYPE_NUMERIC);
//							cell.setCellValue(obj.getTab());
							cell.setCellValue(obj.getCourseteacher());
						}
						if(j==4){
							cell = row.createCell(j,HSSFCell.CELL_TYPE_NUMERIC);
//							cell.setCellValue(obj.getTab());
