...
public class ExportteacherServlet extends HttpServlet {
	//显示的导出表的标题
			private String title="教师列表";
			//导出表的列名
			private String[] rowsName= new String[]{"序号","教工号","姓名","密码","年龄","部门","教师标记"};
		...
		public void doGet(HttpServletRequest request, HttpServletResponse response,List<User> list) throws ServletException, IOException {
			
			//list数据
//			ExcelImportService ex=new ExcelImportService();
			SecService ex=new SecService();
//			dataList=ex.find();
			dataList=list;
			System.out.println(dataList.size()+"赋值后大小");
			for (User teacherBean : dataList) {
				System.out.println(teacherBean+"//");
			}
			ExportteacherServlet ex1 = new ExportteacherServlet();
		...
		public void doPost(HttpServletRequest request, HttpServletResponse response,List<User> list) throws ServletException, IOException {
		 ...
		 * 导出数据
		 * */
		public void export(HttpServletRequest request, HttpServletResponse response,List<User> dataList) throws Exception{
			try{
				...
				for(int i=0;i<dataList.size();i++){
					...
					User obj = dataList.get(i);//遍历每个对象
...
//							cell.setCellValue(obj.getTab());
						...
						if(j==3){
							cell = row.createCell(j,HSSFCell.CELL_TYPE_STRING);
//							cell.setCellValue(obj.getTab());
							cell.setCellValue(obj.getPassword());
						}
						if(j==4){
							cell = row.createCell(j,HSSFCell.CELL_TYPE_STRING);
//							cell.setCellValue(obj.getTab());
						...
						if(j==5){
							cell = row.createCell(j,HSSFCell.CELL_TYPE_STRING);
//							cell.setCellValue(obj.getTab());
							cell.setCellValue(obj.getDepart());
						}
						if(j==6){
							cell = row.createCell(j,HSSFCell.CELL_TYPE_STRING);
//							cell.setCellValue(obj.getTab());
							cell.setCellValue(obj.getState());
						}
